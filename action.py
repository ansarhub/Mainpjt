...
def eg(res):
    ...
    df = pd.read_csv(r"C:\\Users\\junaid ansar\\Downloads\\Employee_attrition_prediction\\WA_Fn-UseC_-HR-Employee-Attrition.csv")  
    # print(df.head(7))
    ...
    if sv is None:
        ...
        db.insert("insert into attrition VALUES('"+str(tmp ['xy'] )+"','"+u_name+"','"+att+"')")
...
def adm_reg():
    if request.method == "POST":
        ...
        status = "admin"
        db = Db()
        db.insert("insert into registration VALUES(0,'" + name + "','" + email + "','" + passw + "','" + typ + "')")
        temp1 = db.selectOne("select * from registration where email='" + email + "'")
        xy = temp1['id']
        db.insert("insert into login VALUES('" +str(xy) +"','"+email+"','"+passw+"','" + typ + "','"+status+"')")
        return <script> alert('User Registered');window.location="/emp_log"</script> \\single qoutes
    else:
...
def emp_reg():
    if request.method == "POST":
        ...
        status = "pending"
        db = Db()
        db.insert("insert into register VALUES(0,'" + name + "','" + phone + "','" + gender + "','" + date + "','" + city + "','" + pin + "','" + state + "','" + quali + "','" + prof + "','" + email + "','" + passw + "')")
...
def adm_log():
    ...
    if request.method == "POST":
        ...
        if ss is not None:
            ...
            return <script> alert('Not Found');window.location="/"</script> //single quotes 
    else:
...
def emp_log():
    ...
    if request.method == "POST":
        ...
        pa = request.form['pass']
        db = Db()
        ss = db.selectOne("select * from login where email='" + em + "' and passw='" + pa + "'")
...
def adminhome():
    db = Db()
    d1=db.select("select count(id) from register")
    
    d2=db.select("select count(*) from attrition where att='Employee will stay'")
    d3=db.select("select count(*) from attrition where att='Employee will not stay'")
    return render_template('dashboard_adm.html', data=u_name,data1=d1,d=d2,data2=d3)
...
def add_on():
    ...
    if request.method == 'POST':
     ...
     file = request.files.get ('ab')                   
     if file:
        '''file_path =os.path.join(app.config[])'''
     #df1 = pd.read_exel(file)
       # print(df1)
...
def eg1(res):
    f1 = res.get('fp')
   # df = pd.read_csv(r"C:\\Users\\junaid ansar\\Downloads\\Employee_attrition_prediction\\WA_Fn-UseC_-HR-Employee-Attrition.csv")  
    df1 = pd.read_csv(files_csv["file"])
...
def notifications2():
    db = Db()
    ''' u_name = db.selectOne("select * from register where email='" + email + "'")'''
